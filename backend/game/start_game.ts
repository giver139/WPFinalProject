...
export async function startGame(roomId: number): Promise<GameView> {
  ...
  const newGame = new GameModel({gameId, players: [...room.players], timestamp: Date.now(), currentPlayer: getFirstMove(), ...newBoard, noFlipEatCount: 0, blackPlayer: -1, winPlayer: -1});
  await room.remove();
